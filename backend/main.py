...
# ==============================================================================
# SERVE FRONTEND FILES - DEBUG VERSION
# ==============================================================================

# Get the correct path to frontend
# When running in Docker: /app/ (main.py is here, frontend/ is sibling)
# When running locally: backend/ (main.py is here, need to go up to find frontend/)
APP_DIR = os.path.dirname(os.path.abspath(__file__))

# Try both paths: local (../frontend) and Docker (./frontend)
...
# Mount static files (CSS, JS, images) FIRST
if os.path.exists(static_path):
    app.mount("/static", StaticFiles(directory=static_path), name="static")
    logger.info(f"✓ Frontend static files mounted from {static_path}")
else:
    logger.error(f"✗ Frontend directory NOT found at {static_path}")
    logger.error(f"APP_DIR: {APP_DIR}")
    logger.error(f"Contents: {os.listdir(APP_DIR)}")
...
async def serve_root():
    ...
    logger.info(f"Serving root - index.html path: {index_path}, exists: {os.path.exists(index_path)}")
...
async def serve_predict():
    ...
    logger.info(f"Serving predict - path: {page_path}, exists: {os.path.exists(page_path)}")
...
async def serve_batch():
    ...
    logger.info(f"Serving batch - path: {page_path}, exists: {os.path.exists(page_path)}")
    if os.path.exists(page_path):
        return FileResponse(page_path, media_type="text/html")
